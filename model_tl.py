...
class generator_I(Model):
    ...
    def __init__(self, nc=3, ngf=64, nz=60):     
        super(generator_I,self).__init__()
    
        self.deconv1 = DeConv2d(in_channels=nz, n_filter=ngf * 8, filter_size=(6,6), strides=(1,1), padding='VALID')
        self.batch1 = BatchNorm2d(num_features=ngf * 8, act=tf.nn.relu)
        self.deconv2 = DeConv2d(in_channels=ngf * 8, n_filter=ngf * 4, filter_size=(4,4), strides=(2,2), padding='SAME')
        self.batch2 = BatchNorm2d(num_features=ngf * 4, act=tf.nn.relu)
        self.deconv3 = DeConv2d(in_channels=ngf * 4, n_filter=ngf * 2, filter_size=(4,4), strides=(2,2), padding='SAME')
        self.batch3 = BatchNorm2d(num_features=ngf * 2, act=tf.nn.relu)
        self.deconv4 = DeConv2d(in_channels=ngf * 2, n_filter=ngf, filter_size=(4,4), strides=(2,2), padding='SAME')
        self.batch4 = BatchNorm2d(num_features=ngf, act=tf.nn.relu)
        self.deconv5 = DeConv2d(in_channels=ngf, n_filter=nc, filter_size=(4,4), strides=(2,2), padding='SAME', act=tf.nn.tanh)
...
class getGRU(Model):
    def __init__(self, input_size, hidden_size, dropout=0):
        ...
        self.drop = tl.layers.Dropout(keep = dropout)
    ...
    def forward(self, x, n_frames, inputs):
        outputs = []
